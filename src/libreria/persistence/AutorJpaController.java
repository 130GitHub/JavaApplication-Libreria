...
import libreria.persistence.exceptions.PreexistingEntityException;
...
public class AutorJpaController implements Serializable {
    ...
    public AutorJpaController () {
        this.emf = Persistence.createEntityManagerFactory("JavaApplicationLibreriaPU");
    }
    ...
    public List<Autor> findAutorXnombre(String nombre) {
        EntityManager em = getEntityManager();
        ...
        try {
            List<Autor> autor = em.createQuery("SELECT a FROM Autor a WHERE a.nombre LIKE '"+nombre+"'").getResultList();
            return autor;
        } finally {
            em.close();
        }
